...
fs.readFile(__dirname + "/" + "icreditscoreabi.json", 'utf8', function(err, data) {
	...
	creditscore = creditscoreContract.at('0xd03eeeb050dd3d8ebb2369478693b0ff10525a32');
	...
	creditscore.getID({
		...
		creditscore.generateSeed(lenderID, companyID, bankAccountNumber, "0x0", {
			...
			if (txReceipt.logs.length == 0) {
				...
				creditscore.getSeed(bankAccountNumber, {
					from: bankAccountAddress,
					gas: 500000
				}, function(err, result) {
					if (err) {
...
app.post('/delayonpayment', function(req, res) {
	...
	creditscore.getID({
		...
		creditscore.generateSeed("0x0", _companyID, _bankAccountNumber, CollAgID, {
			from: Owneraddress,
			gas: 500000
		}, function(err, result) {
			if (err) {
			...
			creditscore.getSeed(_bankAccountNumber, {
				from: Owneraddress,
				gas: 500000
			}, function(err, result) {
				if (err) {
...
app.post('/updateArrearTime', function(req, res) {
	...
	creditscore.getID({
		...
		creditscore.generateSeed("0x0", _companyID, _contractIDOrbankAccountNumber, collAgID, {
			from: Owneraddress,
			gas: 500000
		}, function(err, result) {
			if (err) {
			...
			creditscore.getSeed(_contractIDOrbankAccountNumber, {
				from: Owneraddress,
				gas: 500000
			}, function(err, result) {
				if (err) {
	...
	creditscore.getID({
		...
		creditscore.generateSeed(lenderID, companyID, contractID, "0x0", {
			...
			if (txReceipt.logs.length == 0) {
				...
				creditscore.getSeed(contractID, {
					from: companyAccountAddress,
					gas: 500000
				}, function(err, result) {
					if (err) {
