...
fs.readFile(__dirname + "/" + "icreditscoreabi.json", 'utf8', function(err, data) {
	...
	creditscore = creditscoreContract.at('0x0ced8e973e0de87dcca52cee91862f7bc8796197');
...
app.post('/initiateNewB2C', function(req, res) {
	...
	creditscore.initiateNewIPFSB2C(companyID, IPFSHash, {
		from: bankAccountAddress,
		gas: 500000
	}, function(err, result) {
		if (err) {
...
app.post('/addCompanyToBankTxn', function(req, res) {
	...
	creditscore.addIPFSCompanyToBankTx(companyID, IPFSHash, {
		from: bankAccountAddress,
		gas: 500000
	}, function(err, result) {
		if (err) {
