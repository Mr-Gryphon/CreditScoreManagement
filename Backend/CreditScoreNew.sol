...
contract CreditScore is Owned {
    ...
    event SeedGenerated(bytes32 Generator,bytes32 CompanyID);
...
// function updateCreditScore(bytes32 _companyID,uint8 _daysInArrear) public returns (bool)
// {
//     require(CompanyDetails[_companyID].companyID!="");
//     CompanyDetails[_companyID].
// }
 ...
 */
    ...
    // --------------------------------------------------------Transaction-----------------------------------------------------------
/*
 * @dev: add IPFS hash of registration details
 * @param uint256 index : define registration entity type
 * @param string Hash : IPFS Hash of registration data.
 * @param bytes32 ID : ID of register entity.
 * @return bool : isHashAdded
 */
...
/*
 * @dev: get Associate entity ID of requester address.
 */
...
/*
 * @dev: generate seed for encrption of transaction details on intialization of Tx
 * @param bytes32 _lenderID : lenderId who init TX
 * @param bytes32 _companyID : borrower ID
 * @param bytes32 _bankAccountNumberORContractID : contract or bank account address 
 * @param bytes32 _collectionAgencyID : Collection ID when collection agency report delay on tx
 * @return bool : isSeedAdded
 */
function generateSeed(bytes32 _lenderID,bytes32 _companyID, bytes32 _bankAccountNumberORContractID,bytes32 _collectionAgencyID) public returns(bool isSeedAdded) {
  bytes32 ID = UserID[msg.sender];
  require(ID != "");
  if(ID == _lenderID || ID == _collectionAgencyID){
      require(CompanyDetails[_companyID].companyID!="");
      bytes32 seedsKey = keccak256(abi.encode(_lenderID, _bankAccountNumberORContractID,_collectionAgencyID));

      require(Seeds[seedsKey].companyID=="");
      ...
      Seeds[seedsKey] = newEncryptionSeed;
      emit SeedGenerated(ID,_companyID);
      return true;
  } else{
      return false;
  }
...
/*
 * @dev: get seed associate with bankaccountaddress or contract id
 * @param bytes32 _bankAccountNumberORContractID : BankAccountAddress or ContactID
 * @return bytes32 seed : seed for encryption
 */
function getSeed(bytes32 _lenderID,bytes32 _bankAccountNumberORContractID,bytes32 _collectionAgencyID) public constant returns(bytes32 seed){
  require( UserID[msg.sender]!="" || msg.sender == owner);
  bytes32 seedsKey = keccak256(abi.encode(_lenderID, _bankAccountNumberORContractID,_collectionAgencyID));
  require(msg.sender == owner || UserID[msg.sender]==Seeds[seedsKey].lenderID || UserID[msg.sender]==Seeds[seedsKey].companyID || UserID[msg.sender]==Seeds[seedsKey].collectionAgencyID );
  
  return(Seeds[seedsKey].randomSeed);
}

/*
 * @dev: Intiate new Tx Between Bank to Company
 * @param bytes32 _companyID : Company ID
 * @param uint256 _bankAccountNumber : Bank Account Number
 * @param string Hash : IPFS Hash of Tx Init data.
 * @return : bool _initiate : account create or not
 */
function initiateNewIPFSB2C(bytes32 _companyID,bytes32 _bankAccountNumber, string Hash)  public returns (bool _initiate)
{
    ...
    newIPFSTransaction.bankAccountNumber = _bankAccountNumber;
...
/*
 * @dev: add new Tx Between Bank to Company
 * @param bytes32 _companyID : Company ID
 * @param uint256 _bankAccountNumber : Bank Account Number
 * @param string Hash : IPFS Hash of Transaction data.
 * @return : bool _initiate : account create or not
 */
 ...
 * @param bytes32 _company2ID : Company ID Borrower.
 * @param bytes32 _contractID : Contract ID
 * @param string Hash : IPFS Hash of Init Transaction data.
 * @return : bool _initiate : contract added or not
 */
function initiateNewIPFSC2C(bytes32 _company2ID, bytes32 _contractID, string Hash)  public returns (bool _initiate)
{
    ...
    newIPFSTransaction.Hash = Hash;
    newIPFSTransaction.contractID = _contractID;
    IPFSTransactionDetailsC2C[_company2ID].push(newIPFSTransaction);
...
/*
 * @dev: Add new Contract Tx Between Company to Company
 * @param bytes32 _company2ID : Company ID Borrower.
 * @param bytes32 _contractID : Contract ID
 * @param string Hash : IPFS Hash of add contract Transaction data.
 * @return : bool _initiate : contract added or not
 */
 ...
 * @param string _hash : IPFS hash of delay transaction details.
 */
    function updateArrearTime(bytes32 _lenderID,bytes32 _companyID,bytes32 _contractIDOrbankAccountNumber,string _hash) public{
          ...
          TransactionIPFSCA2C memory newTransactionIPFSCA2C;
            newTransactionIPFSCA2C.lenderID=_lenderID;
            ...
            newTransactionIPFSCA2C.contractID = _contractIDOrbankAccountNumber;
 ...
 * @param string _hash : IPFS Hash of Transaction details.
 */
...
function retriveTransactionB2CIPFS(uint256 _index) public constant returns (bytes32 lenderID, string Hash,bytes32 bankAccountAddress)
{
  bytes32 _companyID = UserID[msg.sender];

  require(CompanyDetails[_companyID].companyAccountAddress==msg.sender || owner==msg.sender );
    return (IPFSTransactionDetailsB2C[_companyID][_index].bankID,IPFSTransactionDetailsB2C[_companyID][_index].Hash,IPFSTransactionDetailsB2C[_companyID][_index].bankAccountNumber);
}
/*
 ...
 * @return : tx details
 */
    function retriveTransactionC2CIPFS(uint256 _index) public constant returns(bytes32 lenderID,string Hash,bytes32 contractID){
      bytes32 _companyID = UserID[msg.sender];

      require(CompanyDetails[_companyID].companyAccountAddress==msg.sender || owner==msg.sender );

        return(IPFSTransactionDetailsC2C[_companyID][_index].company1ID,IPFSTransactionDetailsC2C[_companyID][_index].Hash,IPFSTransactionDetailsC2C[_companyID][_index].contractID);
    }
 ...
 * @return : tx details
 */
    function retriveCollectionAgencyTxOnDelay(uint256 _index) public constant returns(bytes32, bytes32 CollectionAgencyID,string Hash,bytes32){
      bytes32 _companyID = UserID[msg.sender];

      require(CompanyDetails[_companyID].companyAccountAddress==msg.sender || owner==msg.sender );
        return(IPFSCollectionAgencyTxOnDelay[_companyID][_index].lenderID,IPFSCollectionAgencyTxOnDelay[_companyID][_index].collectionAgencyID,IPFSCollectionAgencyTxOnDelay[_companyID][_index].Hash,IPFSCollectionAgencyTxOnDelay[_companyID][_index].contractID);
    }
}