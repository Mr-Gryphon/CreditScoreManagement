...
fs.readFile(__dirname + "/" + "icreditscoreabi.json", 'utf8', function(err, data) {
	...
	creditscore = creditscoreContract.at('0x3886f96af69cced9cf98bd91fd657b4feb29fc45');
					...
					if (decodedLogs[0].name === "BankAdded") {
						...
						*/
						ipfs.add(JSON.stringify(ipfsOutput), (err, rst) => {
							...
							creditscore.addIPFSHash(1, IPFSHASH, _bankID, {
								gas: 500000
							}, function(err, result) {
								if (err) {
									console.log(err);
				...
				if (txReceipt.logs.length > 0) {
					...
					if (decodedLogs[0].name === "CollectionAgencyAdded") {
						...
						ipfs.add(JSON.stringify(ipfsOutput), (err, rst) => {
							console.log(rst);
							IPFSHASH = rst;
							creditscore.addIPFSHash(2, IPFSHASH, _collectionAgency, {
								gas: 500000
							}, function(err, result) {
								if (err) {
									console.log(err);
				...
				if (txReceipt.logs.length > 0) {
					...
					if (decodedLogs[0].name === "CompanyAdded") {
						...
						ipfs.add(JSON.stringify(ipfsOutput), (err, rst) => {
							...
							creditscore.addIPFSHash(0, IPFSHASH, CUID, {
								gas: 500000
							}, function(err, result) {
								if (err) {
									console.log(err);
...
app.post('/initiateNewB2C', function(req, res) {
	...
	creditscore.getID({from: bankAccountAddress,gas: 500000}, function(err, result) 
	{
		...
		creditscore.generateSeed(lenderID, companyID, bankAccountNumber,"0x0", {
			...
			console.log("Seed generation Tx: " + result);
			printTransaction(result);
			var txReceipt = web3.eth.getTransactionReceipt(result);

			if (txReceipt.logs.length == 0) {

				output["result"] = "Account Exist";
				console.log(output);
				res.end(JSON.stringify(output));
			} else {

				creditscore.getSeed(lenderID, bankAccountNumber,"0x0", {
					from: bankAccountAddress,
					gas: 500000
				}, function(err, result) {
					if (err) {
					...
					console.log("bank : " + bankAccountNumber);
...
app.post('/delayonpayment', function(req, res) {
	...
	creditscore.getID({
		...
		// creditscore.generateSeed(LenderID, _companyID, _bankAccountNumber, "0x0", {
		// 	from: Owneraddress,
		// 	gas: 500000
		// }, function(err, result) {
		// 	if (err) {
...
app.post('/updateArrearTime', function(req, res) {
	...
	_contractIDOrbankAccountNumber = _contractIDOrbankAccountNumber + "ca";
	creditscore.getID({
		from: Owneraddress,
		gas: 500000
	}, function(err, result) {
		...
		creditscore.generateSeed(_lenderID, _companyID, _contractIDOrbankAccountNumber, collAgID, {
			from: Owneraddress,
			gas: 500000
		}, function(err, result) {
			if (err) {
			...
			if (txReceipt.logs.length == 0) {
				...
				output["Seeds"] = "Seeds Already Exist";
				console.log(output);
				res.end(JSON.stringify(output));
			}
			creditscore.getSeed(lenderID, _contractIDOrbankAccountNumber, collAgID, {
				from: Owneraddress,
				gas: 500000
			}, function(err, result) {
				if (err) {
				...
				ipfs.add(ipfsEncOutput, (err, rst) => {
					...
					creditscore.updateArrearTime(lenderID,_companyID, _contractIDOrbankAccountNumber, IPFSHash, {
						from: Owneraddress,
						gas: 500000
					}, function(err, result) {
						if (err) {
...
app.post('/initiateC2CTxn', function(req, res) {
	...
	creditscore.getID({
		...
		creditscore.generateSeed(lenderID, companyID, contractID, "0x0", {
			from: companyAccountAddress,
			gas: 500000
		}, function(err, result) {
			if (err) {
			...
			if (txReceipt.logs.length == 0) {
				...
				output["result"] = "Contract Exist";
				console.log(output);
				res.end(JSON.stringify(output));
			} else {

				creditscore.getSeed(lenderID, contractID, "0x0", {
					from: companyAccountAddress,
					gas: 500000
				}, function(err, result) {
					if (err) {
...
app.post('/AddC2CTxn', function(req, res) {
	...
	creditscore.getSeed(lenderID, contractID, "0x0", {
		...
		ipfs.add(ipfsEncOutput, (err, rst) => {
			...
			creditscore.addIPFSCompanyToCompanyTx(companyID, contractID, IPFSHash, {
				from: companyAccountAddress,
				gas: 500000
			}, function(err, result) {
				if (err) {
				...
				if (result) {
					...
					if (txReceipt.logs.length > 0) {
						...
						if (decodedLogs[0].name === "newIPFSDebitTransactionCompanytoCompany") {
							var query = "INSERT INTO TRANSACTIONC2C (TRANSACTIONID,COMPANY2ID,IPFSHASH) VALUES ('" + result + "','" + companyID + "','" + IPFSHash + "')"
							console.log(query);
							executeQuery(res, query, output)
							res.end(JSON.stringify(output));
...
app.post('/AllTxn', function(req, res) {
	...
	creditscore.totalTransaction(_between, {
		...
		if (no_transaction <= count) {
			no_transaction = count - no_transaction;
		} else {
			no_transaction = 0;
			f = count;
		}
		...
		if (count != 0) {
			// var flag = 0;
			if (_between == 2) {
				for (var i = count - 1; i >= no_transaction; i--) {
					console.log("Hello CollectionAgency: " + i);
					creditscore.retriveCollectionAgencyTxOnDelay(i, {
						...
						if (result) {
							...
							creditscore.getSeed(result[0],result[3],result[1], {
								from: address,
								gas: 500000
							}, function(err, seedResult) {
								...
								var seedData = seedResult;


								ipfs.cat(result[1], (err, rstt) => {
									output[f] = [];
				...
				for (var i = count - 1; i >= no_transaction; i--) {
					...
					creditscore.retriveTransactionB2CIPFS(i, {
						...
						if (result) {
							...
							creditscore.getSeed(result[0],result[2],"0x0", {
								from: address,
								gas: 500000
							}, function(err, seedResult) {
				...
				for (var i = count - 1; i >= no_transaction; i--) {
					...
					creditscore.retriveTransactionC2CIPFS(i, {
						...
						// console.log(result);
						if (result) {
							creditscore.getSeed(result[0],result[2], {
								from: address,
								gas: 500000
							}, function(err, seedResult) {
